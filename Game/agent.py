...
class QLearning:
    ...
    def _initializeQTable(self):
        ...
        data = np.zeros((len(self.action_space), len(self.state_space)))
        self.Q = pd.DataFrame(data, index=self.action_space, columns=self.state_space)
    ...
    def train(self):
        ...
        for episode in range(self.episodes):
            ...
            while not gameover:
                ...
                next_q_value = self.Q[next_state].max()
                self.Q[state][action] = old_q_value + self.alpha * (
...
if __name__ == "__main__":
    ...
    agent = QLearning(game, episodes=100)
    print(agent.Q)
    agent.train()
    print(agent.Q)
    agent.test()
